...
from hw.embeddings.word2vec.dataloader.torch_dataset import W2VDataset
from hw.embeddings.word2vec.model import W2VBase
from hw.embeddings.word2vec.utils.func import pairwise_cosine_similarity
from hw.embeddings.config_parser.core import DatamoduleConfig
from hw.tools import conventions
from hw.tools.utils import MATPLOTLIB_COLORS
from hw.tools.utils import setup_pipeline
...
def show_closest_pairs_for_each_word(
    ...
    for sim_index, word_index in enumerate(sampled_indices):
        ...
        closest_pair_indices = torch.argsort(sim_matrix[sim_index, :], descending=True)[
            :pairs_per_word
        ]
...
def visualize_embeddings(
    ...
    if dataset.has_labels:
        ...
        label_to_color = {
            label: MATPLOTLIB_COLORS[i] for i, label in enumerate(unique_labels)
        }
    ...
    if unique_labels is None:
        ...
        for label in unique_labels:
            ...
            label_indices = [
                i for i, w in enumerate(words) if dataset.labels[w] == label
            ]
            plt.scatter(
                embeddings[label_indices, 0],
...
def semantics_test(model: W2VBase, dataset: W2VDataset) -> None:
    """
    ...
    for combination in combinations:
        ...
        if any(word not in dataset.vocab for word in arg_words + [end_word]):
            logger.warning(
                "Did not find all required words in vocabulary. Skipping...."
            )
            continue
        ...
        cosine_sim = float(
            torch.nn.functional.cosine_similarity(
                pseudo_end_vector.unsqueeze(0), end_vector.unsqueeze(0)
            ).item()
        )
        logger.info(
            f'Similarity between vector("{start_word}") - vector("{minus_word}") + vector("{plus_word}") '
        ...
        logger.info(
            f'Closest pairs to pseudo "{end_word}" vector is: {closest_pairs_str}'
        )


def get_word_after_adversarial(s):
    parts = s.split("_")
    try:
        # Find the index of 'adversarial' and get the next word
        idx = parts.index("adversarial")
        return parts[idx + 1] if idx + 1 < len(parts) else None
    except ValueError:
        # 'adversarial' not found in the string
        return None


@hydra.main(config_path=CONFIG_PATH, config_name="w2v_sg_wiki_text_2.yaml")
def main(cfg: DictConfig) -> None:
    ...
    if cfg.analysis.semantics_test.enable:
        logger.info(
            "Performing simple semantic test... This test is specialized for Shakespeare and might not work for other datasets."
        )
        assert (
            not cfg.datamodule.is_graph
        ), "Semantics test is not supported for graph datasets!"
        semantics_test(
            model=pl_trainer.model,
            dataset=dataset,
        )
