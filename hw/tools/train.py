...
def check_train_experiment_history(
    output_dir: str, dataset_name: str, experiment: str
) -> None:
    """
    ...
    """
    exp_tb_logs_dirpath = conventions.get_tb_logs_experiment_path(
        output_dir, dataset_name, experiment
    )
    exp_checkpoints_dirpath = conventions.get_checkpoints_experiment_path(
        output_dir, dataset_name, experiment
    )
    ...
    if any(os.path.exists(dirpath) for dirpath in dirpaths):
        logger.warning(
            f'Experiment "{experiment}" already has some history. Do you want to delete it?'
        )
...
@hydra.main(config_path=CONFIG_PATH, config_name="cora")
def main(cfg: DictConfig) -> None:
    ...
    tb_logger = TensorBoardLogger(
        save_dir=conventions.get_tb_logs_dirpath(
            cfg.path.output_dir, cfg.datamodule.dataset_name
        ),
        name=cfg.train.experiment,
    )

    checkpoints_dirpath = conventions.get_checkpoints_experiment_path(
