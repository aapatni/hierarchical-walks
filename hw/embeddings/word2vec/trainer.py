...
from hw.embeddings.word2vec.loss import NegativeSamplingLoss
from hw.embeddings.word2vec.model import W2VBase
from hw.embeddings.word2vec.utils import torch_helper
from hw.embeddings.word2vec.utils.meter import MetricMeter
from hw.embeddings.word2vec.utils.sampling import generate_noise_batch
...
class Word2VecTrainer(pl.LightningModule):
    ...
    def forward(
        self, inputs: torch.Tensor, outputs: torch.Tensor, proba: bool = True
    ) -> torch.Tensor:
    ...
    def training_step(self, batch: List[torch.Tensor], *args, **kwargs) -> torch.Tensor:
        ...
        noise = generate_noise_batch(
            outputs.shape[0], outputs.shape[1], self._neg_samples, self._vocab_size
        ).to(outputs)
        noise = noise.view(
            outputs.shape[0], -1
        )  # changing shape for easier matrix multiplication
