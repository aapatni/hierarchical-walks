...
from hw.embeddings.common.path import ASSETS_PATH
from hw.embeddings.graph.random_walk_generator import random_walk_factory
from hw.embeddings.word2vec.dataloader.registry import register_dataset
...
class RandomWalkDataset:
    ...
    def __init__(
        ...
        self._graph = self._walk_generator._graph
    ...
    def _get_current_node(self) -> str:
        ...
        """
        return self._nodes[
            self._index // self._walks_per_node // self._walk_generator._num_graphs
        ]

    def __len__(self) -> int:
        return (
            len(self._graph) * self._walks_per_node * self._walk_generator._num_graphs
        )
    ...
    def __next__(self):
        ...
        assert len(walk.split(" ")) - 1 == self._walk_generator._length
        self._index += 1
        return walk
...
class KarateClubDataset(RandomWalkDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        mapping = {
            node: f"n{node + 1:02d}" for node in graph.nodes
    ...
    def __init__(
        ...
        **kwargs,
        ...
        edge_list = pd.read_csv(
            cora_edges_path, sep="\t", header=None, names=["target", "source"]
        )
        ...
        node_data = pd.read_csv(
            cora_nodes_path, sep="\t", header=None, names=column_names
        )
        ...
        features = {
            k: np.array(v)
            for k, v in node_data[feature_names].T.to_dict("list").items()
        }
...
@register_dataset("graph_ppi")
class PPIDataset(RandomWalkDataset):
    """
    PPI graph dataset.
    Reference: https://graphsandnetworks.com/the-cora-dataset/
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ppi_dirpath = os.path.join(ASSETS_PATH, "ppi")
        G_data: nx.Graph = json.load(open(os.path.join(ppi_dirpath, "ppi-G.json")))
        ...
        feats = np.load(os.path.join(ppi_dirpath, "ppi-feats.npy"))
        features = {}
        for i in range(feats.shape[0]):
            features[str(i)] = feats[i]

        labels = json.load(open(os.path.join(ppi_dirpath, "ppi-class_map.json")))
