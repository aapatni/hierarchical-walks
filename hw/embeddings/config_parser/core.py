...
from hw.embeddings.common.path import RUNS_PATH
from hw.embeddings.split import SplitAlgorithm
from hw.embeddings.word2vec.dataloader.torch_dataset import (
    W2VDataset,
    GraphDataset,
    W2VCollateFunctional,
)
from hw.embeddings.word2vec.model import W2VBase
from hw.embeddings.word2vec.trainer import Word2VecTrainer
...
class TrainConfig:
    ...
    def instantiate_scheduler(self, optimizer: Optimizer) -> Union[dict, LRScheduler]:
        ...
        # Option 2
        assert (
            "scheduler" in self.scheduler
        ), "Missing scheduler object in scheduler configuration."
        scheduler = copy.deepcopy(self.scheduler)
        scheduler["scheduler"] = instantiate(
            OmegaConf.create(scheduler["scheduler"]), optimizer=optimizer
        )
        return scheduler
...
class DatamoduleConfig:
    ...
    def instantiate_dataloader(
        self, dataset: Optional[W2VDataset] = None
    ) -> DataLoader:
        """
...
class ModelAnalysisConfig:
    ...
    closest_pairs: ModelClosestPairAnalysisConfig = field(
        default_factory=ModelClosestPairAnalysisConfig
    )
    visualize_embeddings: ModelVisualizeEmbeddingsAnalysisConfig = field(
        default_factory=ModelVisualizeEmbeddingsAnalysisConfig
    )
    semantics_test: ModelSemanticsTestAnalysisConfig = field(
        default_factory=ModelSemanticsTestAnalysisConfig
    )
...
class GraphDownstreamTaskConfig:
    checkpoint: str = "last.ckpt"
    node_classification: GraphDownstreamNodeClassificationConfig = field(
        default_factory=GraphDownstreamNodeClassificationConfig
    )
    edge_classification: GraphDownstreamEdgeClassificationConfig = field(
        default_factory=GraphDownstreamEdgeClassificationConfig
    )
...
class GlobalConfig:
    ...
    downstream: GraphDownstreamTaskConfig = field(
        default_factory=GraphDownstreamTaskConfig
    )

    def instantiate_model(self, dataset: Optional[W2VDataset] = None) -> W2VBase:
        """
    ...
    def instantiate_trainer(
        ...
        optimizer = (
            self.train.instantiate_optimizer(model.parameters())
            if optimizer is None
            else optimizer
        )
        scheduler = (
            self.train.instantiate_scheduler(optimizer)
